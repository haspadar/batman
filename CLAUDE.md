...
## Глобальные скилы (доступны в любом проекте)

- `/bw-secret get <key>` — прочитать секрет из Bitwarden (Secure Note)
- `/bw-secret set <key> <value>` — создать или обновить секрет
- `/bw-secret delete <key>` — удалить секрет

Namespace определяется автоматически по имени проекта из `git remote`. Полное имя записи: `<project>/<key>`.
